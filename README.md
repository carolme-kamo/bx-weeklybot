...
> `SLACK_BOT_TOKEN`은 carol.me에게 Slack DM으로 받으세요.
...
```bash
git clone https://github.com/carol-me/bxd-weekly-bot.git
cd bxd-weekly-bot
```
...
SLACK_BOT_TOKEN=carol에게받은토큰      ← 공통 (carol.me에게 요청)
```
...
```bash
BOT_DIR="$HOME/Desktop/bxd-weekly-bot"
(crontab -l 2>/dev/null; echo "@reboot cd $BOT_DIR && /opt/miniconda3/bin/python3 -u $BOT_DIR/bot.py >> $BOT_DIR/bot.log 2>&1") | crontab -
```

> Python 경로 확인: `which python3`  
> 결과가 다르면 `/opt/miniconda3/bin/python3` 부분을 해당 경로로 교체
